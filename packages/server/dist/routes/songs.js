...
router.get("/", (_, res) => {
  import_song_svc.default.index().then((list) => res.json({ songs: list })).catch((err) => res.status(500).send(err));
});
router.get("/:id", (req, res) => {
  const { id } = req.params;
  import_song_svc.default.get(id).then((song) => res.json(song)).catch((err) => res.status(404).send(err));
});
router.post("/", (req, res) => {
...
router.put("/:id", (req, res) => {
  const { id } = req.params;
  const newSong = req.body;
  import_song_svc.default.update(id, newSong).then((song) => res.json(song)).catch((err) => res.status(404).end());
});
